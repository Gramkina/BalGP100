...
<html><div class="wrapper">
	<head>
		<title>О поликлинике - Специалисты</title>
...
<div>	
	...
	<body>
		<div class="path mediumTextBlue"><span><a href="/">Главная</a> • <a href="/about">О поликлинике</a> • <a href="/about/specialisten.php">Специалисты</a></span></div>
		<div class='content'>
			<div class="veryLargeTextBlack nazvanie">Специалисты</div>
			...
			<div class='infoAdmin'>
				...
				$result = $mysqli->query('SELECT * FROM Doctors WHERE RankNumber = 1 OR RankNumber = 3 ORDER BY RankNumber');
				while($arr = $result->fetch_assoc()){ 
					...
					<div class="mainBlockDivUser">
						...
						<div class="mainBlockDivUserBottom">
			...
			if($adminOption){
			//-------------------------------------?>
			<div class="deleteDoctorsTableAdmin">
				<table>
					<tr class="mediumTextWhite" bgcolor="#2C71C1">
						<td>Должность</td><td colspan="2">ФИО</td>
					</tr>
					<?php 
						$result = $mysqli->query('SELECT Card, Rank FROM Doctors WHERE RankNumber = 1 OR RankNumber = 3');
						$colorTable = 0;
						while($arr = $result->fetch_assoc()){
							$res = $mysqli->query('SELECT Fam, Imya, Otch FROM Users WHERE Card = '.$arr['Card'])->fetch_assoc();
							echo '<tr bgcolor="'.($colorTable++ % 2 ? '#EEEBEB' : '#F5F5F5').'" class="mediumTextBlack"><td>'.$arr['Rank'].'</td><td>'.$res['Fam'].' '.$res['Imya'].' '.$res['Otch'].'</td><td><img card="'.$arr['Card'].'" class="krestikDelete" src="/icons/krestik.svg"></td></tr>';
						}
						$mysqli->close();
					?>
				</table>
